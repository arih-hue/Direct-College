...
export const collegeService = {
  async list(query: CollegeListQuery) {
    ...
    const cacheKey = `colleges:list:${stableCacheKey({ ...query, pageSize, page: query.page })}`;
    ...
    await appCache.set(cacheKey, JSON.stringify(result), LIST_TTL_SECONDS);
    return result;
  },

  async getByIdentifier(identifier: string) {
    const cacheKey = `colleges:one:${identifier}`;
    const cached = await appCache.get(cacheKey);
    if (cached) {
    ...
    if (row) {
      await appCache.set(cacheKey, JSON.stringify(row), 60);
    }
    return row;
  },
};

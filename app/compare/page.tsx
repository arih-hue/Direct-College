...
const comparisonFields = [
  ...
  { key: 'ranking', label: 'NIRF Ranking', icon: Award, highlight: 'lower' },
  { key: 'location', label: 'Location', icon: MapPin },
  { key: 'established', label: 'Established', icon: null },
  { key: 'avgPackage', label: 'Avg Package', icon: TrendingUp, format: 'currency', highlight: 'higher' },
  { key: 'medianPackage', label: 'Median Package', icon: TrendingUp, format: 'currency', highlight: 'higher' },
  { key: 'highestPackage', label: 'Highest Package', icon: TrendingUp, format: 'currency', highlight: 'higher' },
  { key: 'placementRate', label: 'Placement Rate', icon: Users, format: 'percent', highlight: 'higher' },
  { key: 'fees', label: 'Total Fees', icon: IndianRupee, format: 'currency', highlight: 'lower' },
  { key: 'rating', label: 'Rating', icon: Star, highlight: 'higher' },
  { key: 'reviewCount', label: 'Reviews', icon: null },
]
...
export default function ComparePage() {
  ...
  const getBestValue = (key: string, highlight?: string): number | null => {
    ...
    const values = selectedColleges.map((c) => c[key as keyof College] as number)
                      ...
                      {selectedColleges.map((college, index) => (
                        <th key={college.id} className="p-4 text-center min-w-[200px]">
                          <div className="relative">
                            ...
                            <p className="font-semibold text-foreground">{college.shortName}</p>
                  ...
                  <div className="max-h-64 overflow-y-auto space-y-2">
                    {filteredColleges.map((college) => (
                      ...
                      >
                        ...
                        <div>
                          <p className="font-medium text-foreground">{college.shortName}</p>
                          <p className="text-sm text-muted-foreground">{college.location}</p>
                        </div>
                      </button>
                    ))}
